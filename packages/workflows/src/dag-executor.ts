...
export async function executeDagWorkflow(
        ...
        try {
          ...
          if (node.when !== undefined) {
            ...
            if (!conditionParsed) {
              ...
              await logNodeError(logDir, workflowRun.id, node.id, parseErrMsg).catch(
                (err: Error) => {
              ...
              return {
                nodeId: node.id,
                output: { state: 'failed' as const, output: '', error: parseErrMsg },
              };
            }
            if (!conditionPasses) {
  ...
  try {
    await deps.store.completeWorkflowRun(workflowRun.id, {
      ...
      ...(totalTokensIn > 0 ? { total_input_tokens: totalTokensIn } : {}),
      ...(totalTokensOut > 0 ? { total_output_tokens: totalTokensOut } : {}),
    });
  } catch (dbErr) {
    getLog().error(
